...
pub use discover::registry::rewrite_command_with_proxy;
...
fn run_fallback(parse_error: clap::Error, argv: &[OsString]) -> Result<i32> {
    // `argv` is the full command line given to the entrypoint (binary name
    // first). Hosted callers pass a synthetic argv that differs from
    // `std::env::args()`, so never re-read the process arguments here.
...
fn i32_to_exit_code(code: i32) -> ExitCode {
    if code == 0 {
        ExitCode::SUCCESS
...
fn run_cli_from(args: impl IntoIterator<Item = OsString>) -> Result<i32> {
    ...
    let cli = match Cli::try_parse_from(&argv) {
        ...
        Err(e) => {
            ...
            return run_fallback(e, &argv);
        }
    };

    // Warn if installed hook is outdated/missing (1/day, non-blocking).
    // Skip for Gain — it shows its own inline hook warning.
    if !hosted && !matches!(cli.command, Commands::Gain { .. }) {
    ...
    if !hosted && is_operational_command(&cli.command) {
...
fn hosted_mode() -> bool {
    cfg!(feature = "hosted") || std::env::var("RTK_HOSTED").unwrap_or_default() == "1"
}
...
mod tests {
    ...
    /// Hosted entrypoints (`cli_entry`) pass a synthetic argv that differs
    /// from the real process argv. The fallback path for non-subcommands
    /// must execute the passed argv — under `cargo test` the process argv
    /// belongs to the test harness, so re-reading `std::env::args()` there
    /// executes the wrong command (exit 127 instead of 0).
    #[test]
    #[cfg(unix)]
    fn test_run_fallback_uses_passed_argv() {
        // `true` is not an rtk subcommand, so parsing fails and the command
        // is dispatched through run_fallback.
        let argv: Vec<OsString> = ["rtk", "true"].iter().map(OsString::from).collect();
        let parse_error = match Cli::try_parse_from(&argv) {
            Ok(_) => panic!("`true` must not parse as an rtk subcommand"),
            Err(e) => e,
        };

        let code = run_fallback(parse_error, &argv).expect("fallback should execute `true`");

        assert_eq!(code, 0, "fallback must run `true` from the passed argv");
    }
